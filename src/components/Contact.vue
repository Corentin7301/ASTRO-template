<template>
  <div class=" max-h-8">
    <button @click="isOpen = !isOpen" @keydown.esc="escapeKey()" class="focus:outline-white"><svg class="w-8 h-8 text-lightColor hover:opacity-75 transition-all" fill="none"
    ...
    <div v-show="isOpen" @keydown.esc="escapeKey()"
      class=" bg-persoMainColor w-[85%] lg:w-1/3 z-50 fixed top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 rounded-md py-5 px-6 transition-all shadow-xl">
...
<style scoped lang="scss">
  ...
  input[type=submit] {
    ...
    &:hover {
      @apply text-lightColor bg-persoMainColor border-lightColor transition-all;
    }
  }
</style>